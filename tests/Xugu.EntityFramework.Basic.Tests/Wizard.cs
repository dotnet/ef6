...
namespace Xugu.EntityFramework.Tests
{
    ...
    public class WizardTests : DefaultFixture
    {
        ...
        {
            XGProviderServices services = new XGProviderServices();
            string token = services.GetProviderManifestToken(Connection);

            Assert.AreEqual("12.0.0", token);
        }

        [Test]
        public void GetStoreSchemaDescriptionDoesNotThrowForServer50OrGreater()
        {
            XGProviderManifest manifest = new XGProviderManifest(Version.Major + "." + Version.Minor);
