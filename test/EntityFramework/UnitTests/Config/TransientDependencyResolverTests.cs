...
namespace System.Data.Entity.Config
{
    ...
    public class TransientDependencyResolverTests : TestBase
    {
        ...
        {
            [Fact]
            public void GetServices_returns_unnamed_instance_of_contract_interface()
            {
                Assert.NotNull(new TransientDependencyResolver<IPilkington>(() => new Karl()).GetServices<IPilkington>().Single());
            }

            [Fact]
            public void GetServices_returns_named_instance_of_contract_interface()
            {
                Assert.NotNull(new TransientDependencyResolver<IPilkington>(() => new Karl(), "Karl").GetServices<IPilkington>("Karl").Single());
            }

            [Fact]
            public void GetServices_returns_empty_list_when_contract_interface_does_not_match()
            {
                Assert.Empty(new TransientDependencyResolver<IPilkington>(() => new Karl()).GetServices<IGervais>());
                Assert.Empty(new TransientDependencyResolver<IPilkington>(() => new Karl(), null).GetServices<IGervais>());
                Assert.Empty(new TransientDependencyResolver<IPilkington>(() => new Karl(), "Karl").GetServices<IGervais>("Karl"));
            }

            [Fact]
            public void GetServices_returns_empty_list_when_name_does_not_match()
            {
                Assert.Empty(new TransientDependencyResolver<IPilkington>(() => new Karl(), "Karl").GetServices<IPilkington>("Ricky"));
                Assert.Empty(new TransientDependencyResolver<IPilkington>(() => new Karl(), "Karl").GetServices<IPilkington>());
                Assert.Empty(new TransientDependencyResolver<IPilkington>(() => new Karl(), "Karl").GetServices<IGervais>("Ricky"));
            }
