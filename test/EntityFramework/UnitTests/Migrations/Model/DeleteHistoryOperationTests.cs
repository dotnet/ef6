    ...
    public class DeleteHistoryOperationTests
    {
        ...
        public void Ctor_should_validate_preconditions()
        {
            Assert.Equal(
                ...
                Assert.Throws<ArgumentException>(() => new DeleteHistoryOperation(null, "Migration1")).Message);

            Assert.Equal(
                new ArgumentException(Strings.ArgumentIsNullOrWhitespace("migrationId")).Message,
                Assert.Throws<ArgumentException>(() => new DeleteHistoryOperation("Foo", null)).Message);
        }
    }
}
