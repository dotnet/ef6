namespace System.Data.Entity.Core.Objects
{
    ...
    internal class InternalObjectStateManager
    {
        ...
        /// </summary>
        internal InternalObjectStateManager()
        {
        }
        ...
        public InternalObjectStateManager(MetadataWorkspace metadataWorkspace)
        {
        ...
        /// </summary>
        internal ObjectStateManager ObjectStateManagerWrapper { get; set; }
        ...
        internal virtual EntityEntry AddKeyEntry(EntityKey entityKey, EntitySet entitySet)
        {
            ...
            entry = new EntityEntry(entityKey, entitySet, ObjectStateManagerWrapper, typeMetadata);
            ...
            IEntityWrapper wrappedObject, EntityKey passedKey, EntitySet entitySet, string argumentName, bool isAdded)
        {
            ...
            else
            {
                ...
                ObjectStateManagerWrapper.OnObjectStateManagerChanged(CollectionChangeAction.Add, newEntry.Entity);
            ...
            bool replacingEntry)
        {
            ...
            ObjectStateManagerWrapper.OnObjectStateManagerChanged(CollectionChangeAction.Add, keyEntry.Entity);
        }
            ...
            EntityKey sourceKey, AssociationEndMember sourceMember)
        {
            ...
            var deletedRelationships = new List<RelationshipEntry>(ObjectStateManager.InitialListSize);
            ...
            {
                foreach (var relationshipEntry in ObjectStateManagerWrapper.FindRelationshipsByKey(sourceKey))
                {
        ...
        {
            var entry = new RelationshipEntry(ObjectStateManagerWrapper, desiredState, wrapper);
        ...
        {
            return ObjectStateManagerWrapper.FindRelationshipsByKey(key).ToArray();
        }
        ...
        internal virtual ObjectStateEntry[] GetObjectStateEntriesInternal(EntityState state)
        {
        ...
        public virtual bool TryGetRelationshipManager(object entity, out RelationshipManager relationshipManager)
        {
            ...
            {
                var wrappedEntity = EntityWrapperFactory.WrapEntityUsingStateManager(entity, ObjectStateManagerWrapper);
        ...
        internal virtual void ChangeState(EntityEntry entry, EntityState oldState, EntityState newState)
        {
            ...
            if (newState == EntityState.Detached)
            {
                    ...
                    && !TransactionManager.IsAttachTracking)
                {
                    ...
                    ObjectStateManagerWrapper.OnEntityDeleted(CollectionChangeAction.Remove, wrappedEntity.Entity);
                    ObjectStateManagerWrapper.OnObjectStateManagerChanged(CollectionChangeAction.Remove, wrappedEntity.Entity);
                }
            }
            else
            {
            ...
            if (newState == EntityState.Deleted)
            {
                ...
                if (fireEvent)
                {
                    ...
                    ObjectStateManagerWrapper.OnEntityDeleted(CollectionChangeAction.Remove, entry.Entity);
                    ObjectStateManagerWrapper.OnObjectStateManagerChanged(CollectionChangeAction.Remove, entry.Entity);
                }
            }
        }
